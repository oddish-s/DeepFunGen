...
class OnnxSequenceModel:
    ...
    def __init__(self, model_path: str | Path, prefer_gpu: bool = True) -> None:
        ...
        self.normalization_mean = float(getattr(self, "NORMALIZATION_MEAN", 0.0))
        self.normalization_std = float(getattr(self, "NORMALIZATION_STD", 10.0))
    ...
    def _apply_metadata(self) -> None:
        try:
            ...
            return
        custom = getattr(meta, "custom_metadata_map", None)
        if not custom:
            return
        props = {str(key).lower(): str(value) for key, value in custom.items()}

        def _parse_int(*keys: str) -> Optional[int]:
            for key in keys:
                value = props.get(key)
                if value is None:
                    continue
        ...
        def _parse_float(*keys: str) -> Optional[float]:
            for key in keys:
                value = props.get(key)
                if value is None:
                    continue
        ...
        seq = _parse_int("funtorch5.input_sequence", "input_sequence")
        if seq and seq > 0:
            self.sequence_length = seq
        channels = _parse_int("funtorch5.input_channels", "input_channels")
        if channels and channels > 0:
            self.channels = channels
        height = _parse_int("funtorch5.input_height", "input_height")
        if height and height > 0:
            self.height = height
        width = _parse_int("funtorch5.input_width", "input_width")
        if width and width > 0:
            self.width = width
        mean = _parse_float("funtorch5.normalization_mean", "normalization_mean")
        if mean is not None:
            self.normalization_mean = mean
        std = _parse_float("funtorch5.normalization_std", "normalization_std")
